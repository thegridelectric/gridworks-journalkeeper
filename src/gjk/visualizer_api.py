...
from gwk.models import DataChannelSql, MessageSql, ReadingSql
...
def get_data(request):
    if request.password != valid_password:
        with open("failed_logins.log", "a") as log_file:
            ...
            log_entry += (
                f"Timezone '{request.timezone}', device: {request.user_agent}\n\n"
            )
            log_file.write(log_entry)
        return (
            {"success": False, "message": "Wrong password.", "reload": True},
            0,
            0,
            0,
            0,
    ...
    for key in channels.keys():
        # Check the length
        if (
            len(channels[key]["times"]) != len(channels[key]["values"])
        ...
        min_time_ms = min(list(sorted_times)[0], min_time_ms)
        max_time_ms = max(list(sorted_times)[-1], max_time_ms)
        ...
        channels[key]["times"] = [
            x.replace(tzinfo=None) for x in channels[key]["times"]
        ]
        ...
        if "zone" in channel_name and "gw-temp" not in channel_name:
            if "state" not in channel_name:
                channels[channel_name]["values"] = [
                    x / 1000 for x in channels[channel_name]["values"]
                ]
...
async def get_csv(request: CsvRequest):
    ...
    num_points = int(
        (request.end_ms - request.start_ms) / (request.timestep * 1000) + 1
    )
    ...
    csv_times_dt = [
        x.tz_convert("America/New_York").replace(tzinfo=None) for x in csv_times_dt
    ]
...
async def get_plots(request: DataRequest):
    ...
    if PYPLOT_PLOT:
        line_style = (
            "lines+markers" if "show-points" in request.selected_channels else "lines"
        )
        ...
        if (
            "primary-pump-pwr" in request.selected_channels
            and "primary-pump-pwr" in channels
        ):
            power_plot = True
            fig.add_trace(
                go.Scatter(
        ...
        if temp_plot and power_plot:
            fig.update_layout(yaxis=dict(title="Temperature [F]", range=[0, 260]))
            fig.update_layout(
                yaxis2=dict(title="Flow [GPM] or Power [W]", range=[0, 20])
            )
        elif temp_plot and not power_plot:
            fig.update_layout(yaxis=dict(title="Temperature [F]"))
        elif power_plot and not temp_plot:
                ...
                if "temp" in key:
                    fig.add_trace(
                        go.Scatter(
                            ...
                            opacity=0.7,
                            line=dict(
                                color=zone_colors_hex[int(key[4]) - 1], dash="solid"
                            ),
                    ...
                    fig.add_trace(
                        go.Scatter(
                            ...
                            opacity=0.7,
                            line=dict(
                                color=zone_colors_hex[int(key[4]) - 1], dash="dash"
                            ),
        ...
        if "buffer-depths" in request.selected_channels:
            buffer_channels = sorted([
                key
                for key in channels.keys()
                if "buffer-depth" in key and "micro-v" not in key
            ])
            for buffer_channel in buffer_channels:
                yf = [
                    to_fahrenheit(x / 1000) for x in channels[buffer_channel]["values"]
                ]
                ...
                fig.add_trace(
                    go.Scatter(
                        ...
                        line=dict(
                            color=buffer_colors_hex[buffer_channel], dash="solid"
                        ),
                    )
                )
        ...
        if not buffer_channels:
            if (
                "buffer-hot-pipe" in request.selected_channels
                and "buffer-hot-pipe" in channels
            ):
                yf = [
                    to_fahrenheit(x / 1000)
                    for x in channels["buffer-hot-pipe"]["values"]
                ]
                min_buffer_temp = min(min_buffer_temp, min(yf))
                max_buffer_temp = max(max_buffer_temp, max(yf))
                fig.add_trace(
            ...
            if (
                "buffer-cold-pipe" in request.selected_channels
                and "buffer-cold-pipe" in channels
            ):
                yf = [
                    to_fahrenheit(x / 1000)
                    for x in channels["buffer-cold-pipe"]["values"]
                ]
                min_buffer_temp = min(min_buffer_temp, min(yf))
                max_buffer_temp = max(max_buffer_temp, max(yf))
                fig.add_trace(
        ...
        if "storage-depths" in request.selected_channels:
            temp_plot = True
            tank_channels = sorted([
                key for key in channels.keys() if "tank" in key and "micro-v" not in key
            ])
        ...
        if not tank_channels:
            if (
                "store-hot-pipe" in request.selected_channels
                and "store-hot-pipe" in channels
            ):
                temp_plot = True
                yf = [
                    to_fahrenheit(x / 1000)
                    for x in channels["store-hot-pipe"]["values"]
                ]
                min_store_temp = min(min_store_temp, min(yf))
                max_store_temp = max(max_store_temp, max(yf))
                fig.add_trace(
            ...
            if (
                "store-cold-pipe" in request.selected_channels
                and "store-cold-pipe" in channels
            ):
                temp_plot = True
                yf = [
                    to_fahrenheit(x / 1000)
                    for x in channels["store-cold-pipe"]["values"]
                ]
                min_store_temp = min(min_store_temp, min(yf))
                max_store_temp = max(max_store_temp, max(yf))
                fig.add_trace(
        ...
        power_plot = False
        if (
            "store-pump-pwr" in request.selected_channels
            and "store_pump_pwr" in channels
        ):
            power_plot = True
            fig.add_trace(
                go.Scatter(
        ...
        if temp_plot and power_plot:
            fig.update_layout(
                yaxis=dict(
                    title="Temperature [F]",
                    range=[min_store_temp - 40, max_store_temp + 20],
                )
            )
            fig.update_layout(
                yaxis2=dict(title="Flow [GPM] or Power [kW]", range=[-1, 40])
            )
        elif temp_plot and not power_plot:
            fig.update_layout(
                yaxis=dict(
                    title="Temperature [F]",
                    range=[min_store_temp - 20, max_store_temp + 20],
                )
            )
        elif power_plot and not temp_plot:
