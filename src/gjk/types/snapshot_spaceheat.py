...
class SnapshotSpaceheatMaker:
    ...
    @classmethod
    def type_to_tuple(cls, b: bytes) -> SnapshotSpaceheat:
        """
        ...
        Raises:
           GwTypeError: if the bytes are not a snapshot.spaceheat.000 type

        Returns:
            SnapshotSpaceheat instance
        """
        try:
            d = json.loads(b)
    ...
    def dict_to_tuple(cls, d: dict[str, Any]) -> SnapshotSpaceheat:
        """
        Translates a dict representation of a snapshot.spaceheat.000 message object
        ...
        if "Snapshot" not in d2.keys():
            raise GwTypeError(f"dict missing Snapshot: <{d2}>")
        if not isinstance(d2["Snapshot"], dict):
            raise GwTypeError(
                f"Snapshot <{d2['Snapshot']}> must be a TelemetrySnapshotSpaceheat!"
            )
        snapshot = TelemetrySnapshotSpaceheatMaker.dict_to_tuple(d2["Snapshot"])
