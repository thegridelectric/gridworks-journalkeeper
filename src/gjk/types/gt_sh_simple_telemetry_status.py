...
class GtShSimpleTelemetryStatusMaker:
    ...
    @classmethod
    def type_to_tuple(cls, b: bytes) -> GtShSimpleTelemetryStatus:
        """
        ...
        Raises:
           GwTypeError: if the bytes are not a gt.sh.simple.telemetry.status.100 type

        Returns:
            GtShSimpleTelemetryStatus instance
        """
        try:
            d = json.loads(b)
    ...
    def dict_to_tuple(cls, d: dict[str, Any]) -> GtShSimpleTelemetryStatus:
        """
        Translates a dict representation of a gt.sh.simple.telemetry.status.100 message object
