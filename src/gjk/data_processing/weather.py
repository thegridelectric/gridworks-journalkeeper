...
def get_temp(time, coordinates=MILLINOCKET):
    ...
    temperature = x.pick_points(pd.DataFrame(coordinates), method="nearest")[
        "t2m"
    ].values[0]
...
if __name__ == "__main__":
    ...
    plt.plot(
        [np.nan] * (today.hour) + wf_herbie[today.hour :],
        label="Forecast",
        linestyle="dashed",
        color="tab:blue",
    )
    ...
    plt.axvspan(
        today.hour, today.hour + 1, color="orange", alpha=0.1, label="Current hour"
    )
