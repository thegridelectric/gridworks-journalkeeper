...
class AlertGenerator:
    ...
    def update_alert_status(self, message, short_alias, clear_alert=False):
        if clear_alert:
            ...
            new_house_data = {
                "status": HouseStatus(
                    status="alert", message=message, acked=False
                ).to_dict()
            }
        ...
        try:
            ...
            house = (
                session.query(House).filter(House.short_alias == short_alias).first()
            )
    ...
    def send_opsgenie_alert(
        self, message, house_alias, alert_alias, unique_alias=False, priority="P1"
    ):
    ...
    def unix_ms_to_date(self, time_ms):
        return pendulum.from_timestamp(time_ms / 1000, tz=self.timezone_str).replace(
            microsecond=0
        )
    ...
    def get_data_from_journaldb(self):
        ...
        for layout_lite_message in self.layout_lites:
            ...
            self.critical_zones_by_house[llm_house_alias] = {
                "known": True,
                "list": layout_lite_message.payload["CriticalZoneList"],
            }
        ...
        for house_alias in all_house_aliases:
            ...
            for message in [
                m for m in self.messages if m.from_alias.split(".")[-2] == house_alias
            ]:
                    ...
                    if "StateList" in message.payload:
                        for state in message.payload["StateList"]:
                            if "relay" in state["MachineHandle"]:
                                ...
                                if (
                                    state["MachineHandle"]
                                    not in self.relays[house_alias][relay_name]
                                ):
                                    self.relays[house_alias][relay_name][
                                        state["MachineHandle"]
                                    ] = {}
                                ...
                                self.relays[house_alias][relay_name][
                                    state["MachineHandle"]
                                ]["times"].extend(state["UnixMsList"])
                                self.relays[house_alias][relay_name][
                                    state["MachineHandle"]
                                ]["values"].extend(state["StateList"])

            for channel in self.data[house_alias]:
                sorted_times_values = sorted(
                    zip(
                        self.data[house_alias][channel]["times"],
                        self.data[house_alias][channel]["values"],
                    )
                )
            ...
            for relay in self.relays[house_alias]:
                for relay_boss in self.relays[house_alias][relay]:
                    sorted_times_values = sorted(
                        zip(
                            self.relays[house_alias][relay][relay_boss]["times"],
                            self.relays[house_alias][relay][relay_boss]["values"],
                        )
                    )
                    sorted_times, sorted_values = zip(*sorted_times_values)
                    self.relays[house_alias][relay][relay_boss]["times"] = list(
                        sorted_times
                    )
                    self.relays[house_alias][relay][relay_boss]["values"] = list(
                        sorted_values
                    )
        ...
        for house_alias in self.selected_house_aliases:
            ...
            for active_critical_glitch in self.alert_status[house_alias][alert_alias]:
                if (
                    time.time()
                    - self.alert_status[house_alias][alert_alias][
                        active_critical_glitch
                    ]
                    > self.hours_back * 60 * 60
                ):
        ...
        try:
            ...
            with next(get_db()) as session:
                ...
                if not glitches:
                    ...
                    critical_glitches = [
                        g for g in glitches if g.payload["Type"] == "Critical"
                    ]
                    print(
                        f"Found {len(glitches)} glitches, of which {len(critical_glitches)} are critical"
                    )
                for message in glitches:
                    ...
                    if (
                        type == "critical"
                        and unique_id not in self.alert_status[house_alias][alert_alias]
                    ):
                        ...
                        self.alert_status[house_alias][alert_alias][unique_id] = (
                            time_received
                        )
        except Exception as e:
    ...
    def check_no_data(self):
        ...
        for house_alias in self.selected_house_aliases:
            ...
            if not self.data[house_alias]:
                ...
                print(
                    f"- {house_alias}: Found data up to {round((time.time() - most_recent_ms / 1000) / 60, 1)} minutes ago"
                )
                self.alert_status[house_alias][alert_alias] = False
        ...
        for house_alias in self.selected_house_aliases:
            ...
            for zone in channels_by_zone:
                ...
                if self.critical_zones_by_house[house_alias]["known"]:
                    for critical_zone in self.critical_zones_by_house[house_alias][
                        "list"
                    ]:
                ...
                for channel in channels_by_zone[zone]:
                    ...
                    if "temp" in channel and "gw" not in channel:
                        temperature = (
                            self.data[house_alias][channel]["values"][-1] / 1000
                        )
                ...
                if setpoint - temperature < self.max_setpoint_violation_f:
                    ...
                    setpoint_channel: str = [
                        x for x in channels_by_zone[zone] if "set" in x
                    ][0]
                    if (
                        len(set(self.data[house_alias][setpoint_channel]["values"]))
                        == 1
                    ):
                        if not self.alert_status[house_alias][alert_alias][zone]:
                            ...
                            self.send_opsgenie_alert(
                                alert_message, house_alias, alert_alias + f"_{zone}"
                            )
                            self.alert_status[house_alias][alert_alias][zone] = True
                    else:
                        setpoint_values = [
                            x / 1000
                            for x in self.data[house_alias][setpoint_channel]["values"]
                        ]
                        if (
                            min(setpoint_values) < setpoint_values[-1]
                            and min(setpoint_values) - temperature
                            < self.max_setpoint_violation_f
                        ):
                            print(
                                f"-- {zone} is significantly below setpoint but the setpoint was increased recently"
                            )
                        else:
                            if not self.alert_status[house_alias][alert_alias][zone]:
                                alert_message = f"{house_alias}: {setpoint_channel.replace("-set", "")} is significantly below setpoint"
    ...
    def check_dist_pump(self):
        ...
        for house_alias in self.selected_house_aliases:
            ...
            for zone_state in [
                x for x in self.data[house_alias] if "zone" in x and "whitewire" in x
            ]:
                channel = self.data[house_alias][zone_state]
                channel["values"] = [int(abs(x) > threshold) for x in channel["values"]]
                zone_heatcall_times = [
                    t
                    for t, state in zip(channel["times"], channel["values"])
                    if state == 1
                ]
                no_heatcall_times = [
                    t
                ...
                valid_zone_heatcall_times = [
                    t for t in zone_heatcall_times if time.time() - t / 1000 >= 5 * 60
                ]
                if valid_zone_heatcall_times:
                    zone_last_heatcall_time = max(valid_zone_heatcall_times)
                else:
                    continue
                no_heatcall_times_before_heatcall = [
                    t for t in no_heatcall_times if t <= zone_last_heatcall_time
                ]
                if no_heatcall_times_before_heatcall:
                    last_no_heatcall_time_before_heatcall = max(
                        no_heatcall_times_before_heatcall
                    )
                    heatcall_on = [
                        t
                        for t in zone_heatcall_times
                        if t >= last_no_heatcall_time_before_heatcall
                    ]
                    if heatcall_on:
                    ...
                    last_heatcall_length = (
                        max(0, zone_last_heatcall_time - start_of_heatcall) / 1000
                    )
            ...
            if (
                not [x for x in self.data[house_alias] if "zone" in x and "state" in x]
                or "dist-pump-pwr" not in self.data[house_alias]
                or "dist-flow" not in self.data[house_alias]
            ):
            ...
            power_around_heatcall = [
                power
                for time, power in zip(pwr["times"], pwr["values"])
            ...
            if not power_around_heatcall and pwr["values"][-1] <= self.min_dist_pump_w:
                print(
                    f"-- No pump power recorded around heat call and latest power is low"
                )
            elif (
                power_around_heatcall
                and max(power_around_heatcall) <= self.min_dist_pump_w
            ):
            ...
            flow_around_heatcall = [
                flow / 100
                for time, flow in zip(flow["times"], flow["values"])
                if time >= last_heatcall_time - 5 * 60 * 1000
            ]
            if (
                not flow_around_heatcall
                and flow["values"][-1] <= self.min_dist_pump_gpm
            ):
                print(
                    f"-- No pump flow recorded around heat call and latest flow is low"
                )
    ...
    def check_store_pump(self):
        ...
        for house_alias in self.selected_house_aliases:
            ...
            for relay9_boss in self.relays[house_alias]["relay9"]:
                boss_latest_time = max(
                    self.relays[house_alias]["relay9"][relay9_boss]["times"]
                )
            ...
            if (
                "store-pump-pwr" not in self.data[house_alias]
                or "store-flow" not in self.data[house_alias]
            ):
                print(f"{house_alias}: Missing data!")  # TODO: create an alert?
                continue
            ...
            if relay9_state == "RelayClosed":
                if time.time() - time_since_in_current_state / 1000 > 10 * 60:
                    print(
                        f"- {house_alias}: Relay 9 is closed since more than 10 minutes, expecting store flow"
                    )
                    ...
                    if (
                        not power_since_closed
                        and pwr["values"][-1] <= self.min_store_pump_w
                    ):
                        print(
                            f"- {house_alias}: No pump power recorded after relay 9 was closed"
                        )
                    elif max(power_since_closed) <= self.min_store_pump_w:
                        print(
                            f"- {house_alias}: No significant pump power after relay 9 was closed"
                        )
                    else:
                        print(
                            f"- {house_alias}: Found store pump power after relay 9 was closed"
                        )
                    ...
                    if (
                        not flow_since_closed
                        and flow["values"][-1] <= self.min_store_pump_gpm
                    ):
                        print(
                            f"- {house_alias}: No pump flow recorded after relay 9 was closed"
                        )
                    elif max(flow_since_closed) <= self.min_store_pump_gpm:
                        print(
                            f"- {house_alias}: No significant pump flow after relay 9 was closed"
                        )
                    else:
                        print(
                            f"- {house_alias}: Found store pump flow after relay 9 was closed"
                        )
                        self.alert_status[house_alias][alert_alias] = False
                        continue
                    ...
                    if not self.alert_status[house_alias][alert_alias]:
                        ...
                        if found_power_after_closed:
                            ...
                            alert_message += ", and no pump power found"
                        self.send_opsgenie_alert(
                            alert_message, house_alias, alert_alias
                        )
                        self.alert_status[house_alias][alert_alias] = True

            elif relay9_state == "RelayOpen":
                print(
                    f"- {house_alias}: Relay 9 is open, not expecting any store flow at the moment"
                )
                self.alert_status[house_alias][alert_alias] = False
    ...
    def check_hp(self):
        ...
        for house_alias in self.selected_house_aliases:
            ...
            if (
                "hp-idu-pwr" not in self.data[house_alias]
                or "hp-odu-pwr" not in self.data[house_alias]
            ):
                print(f"{house_alias}: Missing data!")  # TODO: create an alert?
                continue
            ...
            for relay5_boss in self.relays[house_alias]["relay5"]:
                boss_latest_time = max(
                    self.relays[house_alias]["relay5"][relay5_boss]["times"]
                )
                if boss_latest_time > latest_relay_time:
                    latest_relay_time = boss_latest_time
            ...
            for relay6_boss in self.relays[house_alias]["relay6"]:
                boss_latest_time = max(
                    self.relays[house_alias]["relay6"][relay6_boss]["times"]
                )
                if boss_latest_time > latest_relay_time:
                    latest_relay_time = boss_latest_time
            ...
            latest_reading_time_ms = max(
                max(odu_channel["times"]), max(idu_channel["times"])
            )
            ...
            on_times_odu = [
                t
                for t, v in zip(odu_channel["times"], odu_channel["values"])
                if v / 1000 >= self.min_hp_kw
            ]
            on_times_idu = [
                t
                for t, v in zip(idu_channel["times"], idu_channel["values"])
                if v / 1000 >= self.min_hp_kw
            ]
    ...
    def check_in_atn(self):
        ...
        for house_alias in self.selected_house_aliases:
            ...
            for relay9_boss in self.relays[house_alias]["relay5"]:
                boss_latest_time = max(
                    self.relays[house_alias]["relay5"][relay9_boss]["times"]
                )
                if boss_latest_time > latest_relay_time:
                    latest_relay_time = boss_latest_time
            ...
            if current_relay5_boss == "a.aa.relay5":
                ...
                current_relay5_boss == "auto.h.n.relay5"
                and not self.alert_status[house_alias][alert_alias]
            ):
                self.send_opsgenie_alert(
                    f"{house_alias}: Not in Atn!", house_alias, alert_alias
                )
                self.alert_status[house_alias][alert_alias] = True
    ...
    def check_hp_on_during_onpeak(self):
        ...
        for house_alias in self.selected_house_aliases:
            ...
            if (
                "hp-odu-pwr" not in self.data[house_alias]
                or "hp-odu-pwr" not in self.data[house_alias]
            ):
                print(f"{house_alias}: Missing data!")  # TODO: create an alert?
                continue
    ...
    def check_rebooting(self):
        ...
        for house_alias in self.selected_house_aliases:
            ...
            layouts_for_house = [
                m
                for m in self.layout_lites
                if m.from_alias.split(".")[-2] == house_alias
            ]
            ...
            if layout_times_for_house:
                rounded_times = [
                    int(t // (5 * 60 * 1000)) for t in layout_times_for_house
                ]
                time_counts = {}
                ...
                if not reboot_detected:
                    print(
                        f"- {house_alias}: No rebooting detected in found layout.lite messages"
                    )
                    self.alert_status[house_alias][alert_alias] = False
            else:
    ...
    def check_alert_status(self):
        ...
        for house_alias in self.selected_house_aliases:
            house_has_an_active_alert = self.check_dict_for_true(
                self.alert_status[house_alias]
            )
            if house_has_an_active_alert:
                ...
                print(
                    f"{house_alias}: No more active alert, clearing any existing alerts"
                )
